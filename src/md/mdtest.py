...
class CTdSpiImpl(api.CThostFtdcTraderSpi):
    ...
    def OnRspQryInstrument(self, pInstrument: api.CThostFtdcInstrumentField,
        ...
        sql = "insert into QUANT_FUTURE_INSTRUMENT (EXCHANGEID,INSTRUMENTNAME,PRODUCTCLASS,DELIVERYYEAR,DELIVERYMONTH,MAXMARKETORDERVOLUME" \
              ...
              "','" + str(pInstrument.DeliveryYear) + "'," + "lpad('" + str(
            pInstrument.DeliveryMonth) + "',2," + "'0')" + \
              "," + str(pInstrument.MaxMarketOrderVolume) + "," + str(pInstrument.MinMarketOrderVolume) + "," + str(
            pInstrument.MaxLimitOrderVolume) + \
              "," + str(pInstrument.MinLimitOrderVolume) + "," + str(pInstrument.VolumeMultiple) + "," + str(
            pInstrument.PriceTick) + ",'" + str(pInstrument.CreateDate) + \
              "','" + str(pInstrument.OpenDate) + "','" + str(pInstrument.ExpireDate) + "','" + str(
            pInstrument.StartDelivDate) + "','" + str(pInstrument.EndDelivDate) + "','" + \
              str(pInstrument.InstLifePhase) + "','" + str(pInstrument.IsTrading) + "','" + str(
            pInstrument.PositionType) + "','" + str(pInstrument.PositionDateType) + \
